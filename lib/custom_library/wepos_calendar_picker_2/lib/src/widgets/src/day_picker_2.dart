...
class DayPicker2 extends StatefulWidget {
...
class _DayPicker2State extends State<DayPicker2> {
  ...
  void initState() {
    super.initState();
    _currentMonth = widget.initialDateTime;
  ...
  @override
  void didUpdateWidget(DayPicker2 oldWidget) {
    super.didUpdateWidget(oldWidget);
    if (widget.initialDateTime != oldWidget.initialDateTime &&
        widget.initialDateTime != _currentMonth) {
      ...
      _ambiguate(WidgetsBinding.instance)!.addPostFrameCallback(
        (Duration timeStamp) => _showMonth(widget.initialDateTime, jump: true),
      );
    }
  }
  ...
  void _handleDirectionFocus(DirectionalFocusIntent intent) {
    setState(() {
      if (_focusedDay != null) {
        ...
        _focusedDay ??= widget.initialDateTime;
      }
    });
  }
...
///
/// This is used by the [DayPicker2] to let its children [_DayPicker]s know
  ...
  Widget build(BuildContext context) {
    ...
    while (day < daysInMonth) {
      ...
      if (day < 1) {
        ...
        final bool isSelectedDay = widget.selectedDates
            .any((DateTime d) => DateUtils.isSameDay(d, dayToBuild));
        ...
        if (widget.config.calendarType == CalendarDatePicker2Type.range) {
          if (widget.selectedDates.length == 2) {
            ...
            final bool isDateInRange = !(dayToBuild.isBefore(startDate) ||
                dayToBuild.isAfter(endDate));
            final bool isStartDateSameToEndDate =
                DateUtils.isSameDay(startDate, endDate);

            if (isDateInRange && !isStartDateSameToEndDate) {
              final BoxDecoration rangePickerIncludedDayDecoration =
                  BoxDecoration(
                color: (widget.config.selectedDayHighlightColor ??
                        selectedDayBackground)
                    .withOpacity(0.15),
